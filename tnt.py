...
async def join_channel():
    try:
        await TNT(JoinChannelRequest("@TNT"))
...
@TNT.on(events.NewMessage(outgoing=True, pattern=r"\.م2"))
async def _(event):
    start = datetime.datetime.now()
    await event.edit(sec2)


@TNT.on(events.NewMessage(outgoing=True, pattern=r"\.م3"))
async def _(event):
    start = datetime.datetime.now()
    await event.edit(sec3)


@TNT.on(events.NewMessage(outgoing=True, pattern=r"\.م4"))
async def _(event):
    start = datetime.datetime.now()
...
@TNT.on(events.NewMessage(outgoing=True, pattern=r"\.اعادة تشغيل"))
